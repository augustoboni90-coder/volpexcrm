// === CONFIGURAZIONE BACKEND (Google Apps Script) ===
...
// === FRASE MOTIVAZIONALE VOLPEX ===
const HERO_QUOTES = [
  'Ogni cliente è una storia: i dati sono il modo migliore per ricordarla.',
  'La creatività ti porta i clienti, i numeri ti dicono come tenerli.',
  'Non vendi solo contratti: costruisci relazioni ricorrenti.',
  'Le decisioni migliori nascono dove si incontrano intuizione e dati.',
  'Un CRM ordinato è come un negozio in vetrina: fa venire voglia di entrare.',
  'Se puoi misurarlo, puoi migliorarlo. Se puoi capirlo, puoi venderlo.',
  'Un cliente seguito bene vale più di dieci sconosciuti.',
  'Il caos è nemico del fatturato: l’ordine è alleato delle idee.',
  'Più conosci i tuoi clienti, meno devi “vendere”.'
];
...
const State = {
  ...
  ui: {
    tab: 'clients',
    search: '',
    modal: null,          // 'client' | 'contract' | 'device' | 'import'
    editing: null,        // record attualmente in modifica
    syncing: false,
    prefill: {
      contract: null,     // precompila cf/nome quando crei contratto da cliente
      device: null        // precompila cf quando crei device da cliente
    },
    importState: {
  ...
  // === INIT: CARICA DA GOOGLE SHEET O CACHE LOCALE ===
  async init() {
    try {
      ...
      // Merge anagrafiche duplicate per codice fiscale
      const byCf = {};
      const normalized = [];
      customers.forEach(c => {
    ...
    // Scegli una frase random per l'hero
    this.ui.heroQuoteIndex = Math.floor(Math.random() * HERO_QUOTES.length);
  },
  ...
  upsertCustomer(p) {
    ...
    if (editing) {
      ...
      if (idx !== -1) {
        ...
        this.data.customers = [
          { id: 'c_' + Date.now(), ...p, codice_fiscale: cf, data_nascita: birth },
          ...list
        ];
      }
    }
  ...
  upsertContract(p) {
    ...
    // Crea/aggiorna anagrafica se non presente
    const cf = (p.codice_fiscale || '').toUpperCase().replace(/\s/g, '');
...
// Data portabilità = 2 giorni lavorativi dopo il primo giorno lavorativo disponibile
function computePortabilityDate(contractDateStr) {
  let d = contractDateStr ? new Date(contractDateStr) : new Date();
  if (isNaN(d.getTime())) d = new Date();
  // se cade di sabato/domenica, sposta a lunedì
...
// === IMPORT DA TESTO: PARSING RAW ===
function buildImportFromRaw(raw) {
  ...
  const contratto = {
    ...
    durata_pagamento: '',
    fornitore: operatore,
    data_portabilita: computePortabilityDate(dataContrattoISO),
    valore_cliente: ''
  };
...
// STEP 1: analizza testo e passa a PREVIEW
function parseAndImportRaw(raw) {
  const res = buildImportFromRaw(raw);
  if (!res) {
    showToast('Impossibile leggere i dati dal testo', 'error');
    return;
  }
  State.ui.importState = {
    step: 'preview',
    raw,
...
// STEP 2: conferma import dopo preview
function confirmImportFromPreview(formEl) {
  const st = State.ui.importState;
  if (!st || st.step !== 'preview') return;

  const fd = new FormData(formEl);
  const c0 = st.customer || {};
  const k0 = st.contract || {};
  const d0 = st.device || null;

  const customer = {
    ...c0,
    codice_fiscale: (fd.get('cliente_cf') || c0.codice_fiscale || '').toUpperCase().replace(/\s/g, ''),
    nome: fd.get('cliente_nome') || c0.nome || '',
    cognome: fd.get('cliente_cognome') || c0.cognome || '',
    telefono: fd.get('cliente_tel') || c0.telefono || '',
    email: (fd.get('cliente_email') || c0.email || '').toLowerCase(),
    data_nascita: fd.get('cliente_nascita') || c0.data_nascita || ''
  };

  const contract = {
    ...k0,
    data: fd.get('contratto_data') || k0.data || '',
    data_attivazione: fd.get('contratto_data') || k0.data_attivazione || '',
    compagnia: fd.get('contratto_compagnia') || k0.compagnia || '',
    tipo_contratto: fd.get('contratto_tipo') || k0.tipo_contratto || '',
    tipo: fd.get('contratto_tipo') || k0.tipo || '',
    nome: customer.nome,
    cognome: customer.cognome,
    numero_contatto: customer.telefono,
    codice_fiscale: customer.codice_fiscale,
    nome_contratto: fd.get('contratto_nome') || k0.nome_contratto || '',
    offerta: fd.get('contratto_nome') || k0.offerta || '',
    numero_lavorato: fd.get('contratto_numero') || k0.numero_lavorato || '',
    numero: fd.get('contratto_numero') || k0.numero || '',
    costo: fd.get('contratto_costo') || k0.costo || '',
    costo_mensile: fd.get('contratto_costo') || k0.costo_mensile || ''
  };
  ...
  if (d0) {
    device = {
      ...d0,
      imei: fd.get('device_imei') || d0.imei || '',
      modello: fd.get('device_modello') || d0.modello || ''
    };
  }

  // reset stato import PRIMA di chiamare upsert
  State.ui.importState = {
    step: 'input',
    raw: '',
    customer: null,
  ...
  if (device) State.upsertDevice(device);
...
function render() {
  ...
  const { tab, search, modal, editing, syncing } = State.ui;
      ...
      <main class="main">
        <div class="top-bar">
          <div class="headline">
            ...
            <div class="headline-sub">Volpex: il cockpit per i tuoi dati.</div>
          </div>
              ...
              tab === 'clients'
                ...
                : tab === 'contracts'
                  ...
                  : tab === 'devices'
                    ...
                    : renderReportsSection(State.data, State.ui.report)
            }
          </section>
...
// HERO VOLPEX
function renderHeroCard(tab) {
  const idx = State.ui.heroQuoteIndex % HERO_QUOTES.length;
  const quote = HERO_QUOTES[idx] || '';
  const subtitle =
    tab === 'clients'   ? 'Parti dal cliente, il resto viene da sé.' :
    tab === 'contracts' ? 'Ogni contratto è un flusso di cassa futuro.' :
    tab === 'devices'   ? 'Ogni device venduto è un motivo per richiamare il cliente.' :
                          'Fatti guidare dai dati, non dalle sensazioni.';
  return `
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;">
        <div>
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
            ...
            <span style="font-weight:600;font-size:14px;">VOLPEX CRM</span>
          </div>
          <div style="font-size:12px;color:#6b7280;">${esc(subtitle)}</div>
        </div>
        <div style="max-width:320px;font-size:12px;color:#4b5563;font-style:italic;">
          “${esc(quote)}”
        </div>
      </div>
    </section>
...
// === TABELLA CLIENTI (con contratti + device + pulsanti dedicati) ===
function renderClientsTable(list, contracts, devices) {
  if (!list.length) {
    return `<div class="empty">Nessun cliente ancora inserito. Usa "Nuovo cliente" per iniziare.</div>`;
  }
  return `
    <table>
      <thead>
      ...
      <tbody>
        ${list.map(c => {
          ...
          const contrNames = contr.slice(0, 3)
            .map(k => k.nome_contratto || k.offerta || '')
            .filter(Boolean)
            .join(', ');

          const devNames = devs.slice(0, 3)
            .map(d => ((d.marca || '') + ' ' + (d.modello || '')).trim())
            .filter(Boolean)
            .join(', ');
        ...
        ${list.map(c => {
          ...
          <tr>
            ...
            <td>
              ...
              <div class="muted">
                ${c.mnp === 'SI' ? 'MNP • ' : ''}${c.telefono_incluso === 'SI' ? 'Tel. incluso' : ''}
              </div>
            </td>
            <td>
